...
def read_arguments():
	...
	parser.add_argument('--early_stop_threshold', type=int, default=20)
...
def main(args):
	...
	if args.do_train:
		...
		train_examples = random.sample(train_examples, 32)
		# take an example
		...
		for epoch in range(args.num_train_epochs):
			...
			if args.do_eval:
				...
				logger.info("Bleu score : {}".format(this_bleu))
...
def calculate_cls_score(file_name, args, tokenizer, device, model, promptTemplate, WrapperClass, output_file_name=None, is_test=False, dev_dataloader=None, best_bleu=None):
	...
	if is_test:
		logger.info("  %s = %s " % ("accuracy", str(this_score)))
	else:
		logger.info("  %s = %s \t Previous best accuracy %s" % ("bleu-4", str(this_score), str(best_bleu)))

	logger.info("  " + "*" * 20)

	return this_score, eval_dataloader
